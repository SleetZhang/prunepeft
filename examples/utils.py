...
def train_text_to_text_model(
    ...
    **kwargs,
    ...
    trainer = TrainerClass(
        ...
        optimizers=kwargs.get("optimizers", (None, None)),
        callbacks=[
            EarlyStoppingCallback(
...
def model_inference_batch(
    ...
    """
    Batch version of model_inference for processing multiple texts at once.

    Args:
        model: The model for inference
        tokenizer: Tokenizer for text processing
        input_texts: List of input texts to process
        model_type: Type of model ("CausalLM" or "ConditionalGeneration")
    ...
    for i in range(0, len(input_texts), batch_size):
        ...
        if model_type == "CausalLM":
            # Tokenize batch with space appended (mimicking single inference)
            batch_with_space = [text + " " for text in batch_texts]
            inputs = tokenizer(
                batch_with_space,
                return_tensors="pt",
                padding=True,
            ...
            for j in range(len(batch_texts)):
                ...
                generated_tokens = outputs.sequences[j][input_len:]
