...
import torch
from transformers import RobertaTokenizer, RobertaForSequenceClassification, AutoTokenizer, Trainer
...
class TrainerWithGrad:
    ...
    def evaluate(self):
        ...
        print(f"Evaluating on validation set... ({total_batches} batches)")
        ...
        try:
            # conf_matrix = confusion_matrix(y_true, y_pred) # 该分数无法序列化存储到json
            pass
        except:
            pass
    ...
    def evaluate_on_testset(self):
        ...
        print(f"Evaluating on test set... ({total_batches} batches)")
        for batch in tqdm(
            ...
            with torch.no_grad():
                ...
                # Handle datasets that may not have 'idx' field
                if 'idx' in batch:
                    idxs.extend(batch['idx'].cpu().numpy())
                else:
                    # Use sequential indices if idx is not available
                    idxs.extend(range(len(batch['label'])))
    ...
    def collate_fn(batch):
        ...
        # Handle datasets that may not have 'idx' field (like ARC datasets)
        if 'idx' in batch[0]:
            idx = torch.tensor([item['idx'] for item in batch])
        else:
            # Create sequential indices for datasets without idx
